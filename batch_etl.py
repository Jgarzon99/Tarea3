...
df = df.filter(col("ts").isNotNull())        .withColumn("vehicle_count", col("vehicle_count").cast(IntegerType()))        .withColumn("avg_speed", col("avg_speed").cast(DoubleType()))

df = df.withColumn("vehicle_count", when(col("vehicle_count") < 0, None).otherwise(col("vehicle_count")))        .withColumn("avg_speed", when((col("avg_speed") < 0) | (col("avg_speed") > 200), None).otherwise(col("avg_speed")))

df_clean = df.na.drop(subset=["vehicle_count","avg_speed"])

df_clean = df_clean.withColumn("date", date_format(col("ts"), "yyyy-MM-dd"))                    .withColumn("hour", hour(col("ts")))                    .withColumn("minute", minute(col("ts")))

df_clean.select("vehicle_count","avg_speed").describe().show()

agg_hour = df_clean.groupBy("city","road_type","date","hour")                    .agg(_sum("vehicle_count").alias("vehicles_sum"),
